...
import TechProfessionals from './components/TechProfessionals';
...
import ZyncJobsDaily from './components/SyncJobsDaily';
...
import ResumeHelpPage from './pages/ResumeHelpPage';
...
import CompleteResumeBuilder from './pages/CompleteResumeBuilder';
import CanvaResumeTemplates from './pages/CanvaResumeTemplates';
...
function App() {
  ...
  useEffect(() => {
    ...
    // Handle URL-based navigation (for reset password links)
  ...
  const handleNavigation = (page: string, topic?: string) => {
    console.log('Navigation called:', { page, topic, currentPage, user });
  ...
  if (currentPage === 'resume-help') {
    return <ResumeHelpPage onNavigate={handleNavigation} />;
  }
  ...
  if (currentPage === 'job-application') {
    return (
      <JobApplicationPage 
        onNavigate={handleNavigation} 
        user={user ? {
          id: '1', // Temporary ID - will be replaced with proper user ID from localStorage
          name: user.name,
          email: 'user@example.com' // Temporary email - will be replaced with proper email from localStorage
        } : null}
      />
    );
  }
  ...
  if (currentPage === 'job-posting') {
    return <JobPostingPage onNavigate={handleNavigation} />;
  }
  ...
  if (currentPage === 'resume-builder') {
    return <ResumeBuilderPage onNavigate={handleNavigation} user={user} onLogout={handleLogout} />;
  }

  if (currentPage === 'resume-templates') {
    return <ResumeTemplateSelector onNavigate={handleNavigation} user={user} onLogout={handleLogout} />;
  }

  if (currentPage === 'ai-resume-builder') {
    return <AIResumeBuilderPage onNavigate={handleNavigation} user={user} onLogout={handleLogout} selectedTemplate={currentData} />;
  }
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-100">
        <Header onNavigate={handleNavigation} user={user} onLogout={handleLogout} />
      <Hero onNavigate={handleNavigation} user={user} onShowNotification={(notif) => setNotification({...notif, isVisible: true})} />
      <Stats />
      <TechProfessionals onNavigate={handleNavigation} user={user} />

      <SalaryReport onNavigate={handleNavigation} />
      <EmployerSolutions onNavigate={handleNavigation} />
      <WhyChoose onNavigate={handleNavigation} />
      <BrowseJobs onNavigate={handleNavigation} user={user} onShowNotification={(notif) => setNotification({...notif, isVisible: true})} />
      <ZyncJobsDaily onNavigate={handleNavigation} />
      <Footer onNavigate={handleNavigation} />
      <ChatWidget />
